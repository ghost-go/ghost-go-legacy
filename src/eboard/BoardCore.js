...
function execPonnuki(array, i, j, ki) {
  const { liberty: libertyUp, recursionPath: recursionPathUp } = calcLiberty(array, i, j - 1, ki);
  const { liberty: libertyDown, recursionPath: recursionPathDown } = calcLiberty(array, i, j + 1, ki);
  const { liberty: libertyLeft, recursionPath: recursionPathLeft } = calcLiberty(array, i - 1, j, ki);
  const { liberty: libertyRight, recursionPath: recursionPathRight } = calcLiberty(array, i + 1, j, ki);
  if (libertyUp === 0) {
...
function canMove(array, i, j, ki) {
  ...
  array[i][j] = ki;
  const { liberty } = calcLiberty(array, i, j, ki);
