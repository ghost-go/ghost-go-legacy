...
export default class Board {
  ...
  fitBoard() {
    ...
    this.offsetX =
      this.rightmost > this.maxhv ? this.rightmost - this.maxhv : 0;
    this.offsetY =
      this.bottommost > this.maxhv ? this.bottommost - this.maxhv : 0;
  }
  ...
  renderCoordinate(ctx) {
    if (this.showCoordinate) {
      ...
      let numbers = [];
      if (
        this.leftmost + this.rightmost < 10 &&
        this.topmost + this.bottommost < 10
      ) {
        ...
        letters = A1_LETTERS.slice(
          A1_LETTERS.length - this.maxhv,
          A1_LETTERS.length
        );
        numbers = A1_NUMBERS.slice(0, this.maxhv);
      } else if (
      ...
      letters.forEach((l, index) => {
        ctx.fillText(l, this.size * (index + 1) - this.size / 8, this.size / 2);
      });
